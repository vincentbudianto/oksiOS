...
void handleTimerInterrupt(int segment, int stackPointer)
{
	...
	// }
	tickCount++;
	if (tickCount > 800 && tickCount < 850){
		printInt(segment >> 12);
	}
	setKernelDataSegment();
	currPCB = getPCBOfSegment(segment);
...
//     setKernelDataSegment();
//     tickCount = ticks;
//     currSegment = running->segment;
//     currPointer = running->stackPointer;
//     restoreDataSegment();
//     // pauseProcess(currSegment, res);

//     while (tickCount > 0)
//     {
//         printInt(tickCount);
//         handleTimerInterrupt(currSegment, currPointer);
//     }

//     // resumeProcess(currSegment, res);
// }
